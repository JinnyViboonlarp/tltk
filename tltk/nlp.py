...
def check_thaidict(Word):
    global TDICT
    # jinny line: check if one-char long
    if(len(Word) <= 1):
        return(0)
    if Word in TDICT:
        return(1)
...
#### Check whether the word existed in the dictionary 
def add_to_thaidict(Word):
    global TDICT
    TDICT[Word] = 1
    
def remove_from_thaidict(Word):
    global TDICT
    if Word in TDICT:
        del TDICT[Word]
...
def wordseg_colloc(Input, jinnyedit = True):
    global TDICT
    global EndOfSent
    global chart
    global SegSep
    ...
    part = Input.split(SegSep)
    for inx in part:
        SylLst = syl_segment(inx, jinnyedit = jinnyedit).split('~')
...
####   spellchk=yes|no 
######################################################################
def word_segment(Input,method='colloc',spellchk='no',jinnyedit=True):
    global SegSep
    global SSegSep
    output = ""
    out = ""
...
#                print('ss:',inp)
                if method == 'mm' or method == 'ngram':
                    ...
                    out =wordseg_colloc(inp, jinnyedit = jinnyedit)
#                elif method == 'ngram':
#                    out =wordseg_mm(inp,method,spellchk)
            output = output+out+WordSep
...
def syl_segment(Input, jinnyedit = True):
    global SegSep
    global SSegSep
    output = ""
    out = ""
...
#        print "s:",s
        ...
        for inp in inLst:
            ...
            if objMatch:
                ...
                out = sylseg(inp, jinnyedit = jinnyedit)
...
####### Segment syllable using trigram statistics, only strings matched with a defined syllable pattern will be created
####  Input = Thai string
def sylseg(Input, jinnyedit = True):
    global SylSep
    global PRON
...
#                            schartx[0][k] = s1+s2 
    ...
    if EndOfInput in schart[0]:    
        ...
        if(not(jinnyedit)):
            return('<Fail>'+Input+'</Fail>')
        # below is edit by Jinny
        if(0 in schart.keys() and (len(schart[0]) > 0)):
            max_valid_index = max(list(schart[0].keys()))
            syls_before = SylSep.join(schart[0][max_valid_index])
            syls_after = sylseg(Input[max_valid_index:])
            syls = [syls_before, syls_after]
        else:
            syls_before = Input[0]
            if(len(Input) > 1):
                syls_after = sylseg(Input[1:])
                syls = [syls_before, syls_after]
            else:
                syls = [syls_before]
        return(SylSep.join(syls))  
